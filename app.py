...
def draw_scatterplot():
    count_of_point: int = 500
    np.random.seed(25)
    random_data_array_x = 200 + 50 * np.random.randn(count_of_point)
    random_data_array_y = 200 + 50 * np.random.randn(count_of_point)
    plt.hist2d(random_data_array_x, random_data_array_y, bins=input.selected_number_of_bins(), density=True, color="grey")
    plt.xlabel('X Value')
    plt.ylabel('Y Value')
    plt.title('My First Random Scatterplot')
